...
class Parser:
    ...
    def nextToken(self):
        print(self.curToken.text if self.curToken else "",end=" ") #########################
    ...
    def program(self):
        ...
        #l = self.l()
        #self.emitter.headerLine(f"jmp {l}")

        #self.emitter.emitLine(f":{l}")
    ...
    def statement(self):
        ...
        # "WHILE" comparison "REPEAT" block "ENDWHILE"
            ...
            l2 = self.l()
            l3 = self.l()
            self.emitter.emitLine(f"jmp {l2}")

            self.emitter.emitLine(f':{l3}')
            self.comparison()
            ...
            self.emitter.emitLine(f":{l1}")

            # Zero or more statements in the loop body.
            while not self.checkToken(TokenType.ENDWHILE):
                self.statement()

            self.emitter.emitLine(f":{l2}")
            self.emitter.emitLine(f'call {l3}')
            self.cndjmp(l1)
        ...
        # "INPUT" ident
            ...
            self.emitter.emitLine('int 0x01')
            self.emitter.emitLine('int 0x02')
            #self.emitter.emitLine('string2int #9')
            try:
                self.emitter.emitLine(f'store #{self.thevars[self.curToken.text]}, #0')
            except:
                raise KeyError(f'{self.curToken.text} is not in {self.thevars.keys()}!')
        ...
        # "STRING" ident
        if self.checkToken(TokenType.TOSTRING):
            self.nextToken()
            if self.checkToken(TokenType.IDENT):
                try:
                    self.emitter.emitLine(f'int2string #{self.thevars[self.curToken.text]}')
                except:
                    raise KeyError(f'{self.curToken.text} is not in {self.thevars.keys()}!')
                self.nextToken()
                #self.nextToken() #########
        ...
        # "INT" ident
        if self.checkToken(TokenType.TOINT):
            print('h')
            self.nextToken()
            if self.checkToken(TokenType.IDENT):
                try:
                    self.emitter.emitLine(f'string2int #{self.thevars[self.curToken.text]}')
                except:
                    raise KeyError(f'{self.curToken.text} is not in {self.thevars.keys()}!')
                self.nextToken()
                #self.nextToken()
    ...
    def cndjmp(self,label):
        if self.isnot:
            self.emitter.emitLine(f'jmpz {label}')
        else:
            self.emitter.emitLine(f'jmpnz {label}')
    ...
    def comparison(self):
        ...
        #print(self.curToken.text)
        #print(self.peekToken.text)
        #print(self.checkToken(TokenType.EQEQ))
        ...
        if self.isComparisonOperator():
            ...
            if self.curToken.text == '==':
                self.isnot = False
                #self.nextToken()
                self.emitter.emitLine('store #7, #8')
                self.nextToken()
                self.expression()
                self.emitter.emitLine('cmp #7, #8')
                ...
                self.nextToken()
                self.expression()
                self.emitter.emitLine('is_more #7, #8')
            elif self.curToken.text == '>':
                self.isnot = True
                #self.nextToken()
                self.emitter.emitLine('store #7, #8')
                ...
                #self.nextToken()
                self.emitter.emitLine('store #7, #8')
                self.emitter.emitLine('inc #7')
                self.nextToken()
                self.expression()
                self.emitter.emit('is_more #7, #8')
            elif self.curToken.text == '>=':
                self.isnot = True
                #self.nextToken()
                self.emitter.emitLine('store #7, #8')
                self.emitter.emitLine('dec #7')
                self.nextToken()
                self.expression()
                self.emitter.emit('is_more #7, #8')
            else:
